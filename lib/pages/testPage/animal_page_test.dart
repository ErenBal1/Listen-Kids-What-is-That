...
import 'package:learning_voices_for_babies/pages/categoryPage/widgets/widgets.dart';

import 'dart:async'; // Timer için ekledik
import 'dart:math';

import 'package:learning_voices_for_babies/pages/mainPage/main_page.dart';

class AnimalPageTest extends StatefulWidget {
  const AnimalPageTest({super.key});

  @override
  State<AnimalPageTest> createState() => _AnimalPageTestState();
}

class _AnimalPageTestState extends State<AnimalPageTest> {
  ...
  void _playSoundAutomatically() {
    ...
    _timer = Timer(autoPlayDuration, () async {
      String audioPath = ("sounds/animals/${animals[dogruCevapIndex]}.mp3");
          ...
          Center(
            child: FloatingActionButton.large(
              ...
              onPressed: () async {
                ...
                String audioPath =
                    ("sounds/animals/${animals[dogruCevapIndex]}.mp3");
                await player.play(AssetSource(audioPath));
  ...
  void _restartPage() {
    Future.delayed(const Duration(milliseconds: 600), () {
      ...
      Navigator.of(context).pushReplacement(
        MaterialPageRoute(
          builder: (BuildContext context) => const AnimalPageTest(),
        ),
      );
    });
  }
